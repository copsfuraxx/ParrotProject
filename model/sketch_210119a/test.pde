...
public class SecondApplet extends PApplet {
 ...
 public void draw(){
  ...
  for (int i = 0; i < menu.getListeBoutonTaille();i++){
    ...
    int  j = 0;
    int k = 1;
    for(int i = 0 ; i < menu.getListeBouton().get(i).getTabTexte.length;i++){
      text(menu.getListeBouton().get(i).getTabTexte[i], menu.getListeBouton().get(i).getTabCoord[j],menu.getListeBouton().get(i).getTabCoord[k]);
      j = j+2;
      k = j+1;
    }
  };
